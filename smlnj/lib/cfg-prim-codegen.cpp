 ...
 */
      ...
      // get a pointer to the beginning of the object that has `char *` type
        llvm::Value *initPtr = cxt->createBitCast (obj, cxt->bytePtrTy);
      ...
      // initialize the object's fields
        ...
        for (int i = 0;  i < len;  ++i) {
          ...
          // get a `char *` pointer to obj+offset
            auto adr = cxt->createGEP (initPtr, offset);
            if (argTy == cxt->mlValueTy) {
              // the field should be a native-sized integer
                ...
                cxt->createStore (
                    args[i],
                    cxt->createBitCast (adr, elemTy->getPointerTo()),
                    szb);
            }
            offset += szb;
        }
    ...
    llvm::Value *RAW_ALLOC::codegen (smlnj::cfgcg::Context *cxt, Args_t const &args)
    {
      ...
      // bump the allocation pointer
        cxt->setMLReg (smlnj::cfgcg::CMRegId::ALLOC_PTR,
            cxt->createBitCast (
                cxt->createGEP (cxt->bytePtrTy, allocPtr, len + cxt->wordSzInBytes()),
                cxt->objPtrTy));
  ...
  /***** code generation for the `pure` type *****/
    ...
    {
        llvm::Value *adr = cxt->createGEP (cxt->asObjPtr(args[0]), cxt->asInt(args[1]));
    ...
    llvm::Value *PURE_RAW_SUBSCRIPT::codegen (smlnj::cfgcg::Context *cxt, Args_t const &args)
    {
        ...
        llvm::Value *adr = cxt->createGEP (elemTy->getPointerTo(), args[0], args[1]);
...
/* QUESTION: should we specialize the case where the offset is 0? */
        llvm::Value *adr = cxt->createBitCast (
            cxt->createGEP (
                cxt->bytePtrTy,
                cxt->asBytePtr (args[0]),
  ...
  /***** code generation for the `looker` type *****/
    ...
    {
        return cxt->build().CreateLoad (cxt->mlValueTy, cxt->asObjPtr(args[0]));

    } // DEREF::codegen

    llvm::Value *SUBSCRIPT::codegen (smlnj::cfgcg::Context *cxt, Args_t const &args)
    {
        llvm::Value *baseAdr = cxt->asObjPtr(args[0]);
...
// QUESTION: should we mark the load as volatile?
    ...
    llvm::Value *RAW_LOAD::codegen (smlnj::cfgcg::Context *cxt, Args_t const &args)
    {
      ...
      // and then bitcast to the desired pointer type for the load
        llvm::Value *adr =
            cxt->createPointerCast (
                cxt->createGEP (cxt->bytePtrTy, args[0], args[1]),
                elemTy->getPointerTo());
...
// QUESTION: should we mark the load as volatile?
  ...
  // utility function to generate a store-list update
    ...
    {
        cxt->createStoreML (args[2], cxt->createGEP (cxt->objPtrTy, args[0], args[1]));

    } // UNBOXED_UPDATE::codegen

    void UPDATE::codegen (smlnj::cfgcg::Context *cxt, Args_t const &args)
    {
        llvm::Value *adr = cxt->createGEP (cxt->objPtrTy, args[0], args[1]);
